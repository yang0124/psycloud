				...
				<c:if test="${user_permissions.can_do_test != 0}">
					<li class="active">
						...
						<ul class="submenu">
							<li class="active">
								<a
							href="${basePath}/testAction/testResult">
									<i
		...
		<div id="page-content" class="clearfix">
			...
			<div class="row-fluid">
				<table id="table_report"
					...
					<tbody>
						<c:forEach var="test_item" items="${test_list }">
							<tr>
								<td>${test_item.id }</td>
								<td>${test_item.title_cn }</td>
								<td>${test_item.start_time }</td>
								<td>${test_item.end_time }</td>
								<td>
									<div class="btn-group btn-group-xs" role="group"
											aria-label="...">
										<button id="${scale_item.id }" type="button"
												class="btn btn-small btn-info"
												onclick='windowOpen("${basePath}/testAction/testRecordDetail?test_id=${test_item.id }&scale_id=${test_item.scale_id }&test_time=${test_item.start_time }","_blank")'>查看详情</button>
									</div>
								</td>
							</tr>
						</c:forEach>
