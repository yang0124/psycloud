...
<script type="text/javascript">
	function addManagerPermission() {
		/* alert($('#addUserOrganizationForm').serialize()); */
		$.ajax({
			cache : true,
			type : "POST",
			url : '${basePath}/userAction/addManagerPermission',
			data : $('#addManagerPermissionForm').serialize(),
			async : false,
	...
	function deleteManagerPermission(manager_permission_id) {
		var _data = {
			"manager_permission_id" : manager_permission_id
		};
		$.ajax({
			cache : true,
			type : "POST",
			url : '${basePath}/userAction/deleteManagerPermission',
			data : _data,
			async : false,
			error : function(request) {
				window.wxc.xcConfirm("系统出错了!",
			...
			<div id="page-content" class="clearfix">
				<div class="page-header position-relative">
					...
					<button class="btn btn-mini btn-primary" data-toggle="modal"
						...
						<i class="icon-pencil"></i> 添加权限
					</button>
				</div>
				<div class="modal fade bs-example-modal-sm" tabindex="-1"
					role="dialog" aria-labelledby="mySmallModalLabel"
					id="addManagerPermission" style="display: none">
						...
						<div class="modal-content">
							...
							<div class="modal-body">
								<form class="form-horizontal" action=" " method="" novalidate
									...
									<div class="control-group">
										<label class="control-label" for="manager_id">教师</label>
										<div class="controls">
									...
									<div class="control-group">
										<label class="control-label" for="team_id">系</label>
										<div class="controls">
									...
									<div class="control-group">
										<label class="control-label" for="sub_team_id">班级</label>
										<div class="controls">
							...
							<div class="modal-footer">
								...
								<button type="button" class="btn btn-primary"
									data-dismiss="modal" onclick="addManagerPermission()">确定</button>
							</div>
						</div>
					</div>
				</div>
				<table id="table_report"
					...
					<tbody>
						<c:forEach var="item" items="${manager_permission_list }">
							<tr>
								<td>${item.manager_name}</td>
								<td>${item.manager_team_title }-${item.manager_sub_team_title}</td>
								<td>
									<div class="btn-group btn-group-xs" role="group"
										aria-label="...">
										<button id="${item.id }" type="button" class="btn btn-small"
											onclick='deleteManagerPermission("${item.id }")'>删除</button>
									</div>
								</td>
							</tr>
						</c:forEach>
