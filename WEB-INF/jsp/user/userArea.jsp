...
<head>
	...
	function addUserArea() {
		$.ajax({
			cache : true,
			type : "POST",
			url : '${basePath}/userAreaAction/addArea',
			data : $('#addUserAreaForm').serialize(),
			async : false,
	...
	<div id="page-content" class="clearfix">
			...
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="myModalLabel">添加大区</h4>
					</div>
					<div class="modal-body">
						<form class="form-horizontal" action=" " method="" novalidate
									id="addUserAreaForm">
							<div class="control-group">
								<label class="control-label" for="user_area_title">大区名称</label>
								<div class="controls">
									<input id="user_area_title" type="text"
												name="user_area_title"></div>
							</div>
							<div class="hr"></div>
							<div class="control-group">
								<label class="control-label" for="user_area_admin_name">大区管理员姓名</label>
								<div class="controls">
									<input id="user_area_admin_name" type="text"
												name="user_area_admin_name"></div>
							</div>
							<div class="control-group">
								<label class="control-label" for="user_area_admin_account">大区管理员账号</label>
								<div class="controls">
									<input id="user_area_admin_account" type="text"
												name="user_area_admin_account"></div>
							</div>
							<div class="control-group">
								<label class="control-label" for="user_area_admin_account">大区管理员密码</label>
								<div class="controls">
									<input id="user_area_admin_password" type="text"
												name="user_area_admin_password"></div>
							</div>
						</form>
					</div>
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
									data-dismiss="modal" onclick="addUserArea()">确定</button>
					</div>
				</div>
			</div>
		</div>
		<div class="row-fluid">
			<table id="table_report"
				...
				<tbody>
					<c:forEach var="area_item" items="${area_list }">
						<tr>
							<td>${area_item.id }</td>
							<td>${area_item.title }</td>
							<td>
								<div class="btn-group btn-group-xs" role="group"
											aria-label="...">
									<button id="${area_item.id }" type="button"
												class="btn btn-small btn-success"
												onclick='windowOpen("${basePath}/scaleAction/scaleDetail?id=${area_item.id }")'>修改</button>
									<button id="${area_item.id }" type="button"
												class="btn btn-small"
